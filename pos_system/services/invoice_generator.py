...
class InvoiceGenerator:
    ...
    def generate_invoice(self, invoice_data, items, customer_data, booking_ref=None):
        ...
        sig_style = ParagraphStyle('Sig', fontSize=10, alignment=TA_CENTER, textColor=colors.HexColor('#333333'))
        
        # Create signature line with label
        sig_line = Table([
            [Paragraph("_" * 28, sig_style)],
        ...
        sig_container = Table([[Spacer(1, 1), sig_line]], colWidths=[page_width - 55*mm, 55*mm])
        sig_container.setStyle(TableStyle([('ALIGN', (1, 0), (1, 0), 'RIGHT')]))
        story.append(sig_container)
        story.append(Spacer(1, 8*mm))
        
        # === TERMS & CONDITIONS (Professional English Policy - CENTERED) ===
        terms_title_style = ParagraphStyle('TermsTitle', fontSize=11, fontName='Helvetica-Bold', alignment=TA_CENTER, textColor=colors.HexColor('#333333'))
        terms_text_style = ParagraphStyle('TermsText', fontSize=10, fontName='Helvetica', alignment=TA_CENTER, textColor=colors.HexColor('#444444'), leading=14)
        ...
        footer_style = ParagraphStyle('Footer', fontSize=11, alignment=TA_CENTER, textColor=colors.HexColor('#333333'))
        social_style = ParagraphStyle('Social', fontSize=10, alignment=TA_LEFT, textColor=colors.HexColor('#555555'), leading=13)
        ...
        # Developer credit - very small and light gray
        story.append(Paragraph("Developed by Malinda Prabath | 076 220 6157", dev_style))
    ...
    def generate_booking_invoice(self, booking_data, created_by_name):
        ...
        table_data = [
            [Paragraph("Description", header_desc_style), Paragraph("Qty", header_center_style), Paragraph("Unit Price", header_right_style), Paragraph("Amount", header_right_style)],
            [Paragraph(service_name, desc_style), Paragraph("1", center_style), Paragraph(f"Rs. {full_amount:,.2f}", right_style), Paragraph(f"Rs. {full_amount:,.2f}", right_style)]
        ]
        ...
        # === TERMS & CONDITIONS (Sinhala Policy - CENTERED) ===
        terms_title_style = ParagraphStyle('TermsTitle', fontSize=11, fontName='Helvetica-Bold', alignment=TA_CENTER, textColor=colors.HexColor('#333333'))
        terms_text_style = ParagraphStyle('TermsText', fontSize=10, fontName=SINHALA_FONT, alignment=TA_CENTER, textColor=colors.HexColor('#444444'), leading=14)
        ...
        # === FOOTER SECTION (Social Media & Contact) ===
        footer_style = ParagraphStyle('Footer', fontSize=11, alignment=TA_CENTER, textColor=colors.HexColor('#333333'))
        social_style = ParagraphStyle('Social', fontSize=10, alignment=TA_CENTER, textColor=colors.HexColor('#555555'), leading=13)
        dev_style = ParagraphStyle('Dev', fontSize=8, alignment=TA_CENTER, textColor=colors.HexColor('#999999'))
        
        story.append(Paragraph("Thank you for your business!", footer_style))
        story.append(Spacer(1, 2*mm))
        story.append(Paragraph("Email: studioshineart05@gmail.com | Facebook: Pasindu P Wijethunga Photography", social_style))
        story.append(Spacer(1, 3*mm))
        story.append(Paragraph("Developed by Malinda Prabath | 076 220 6157", dev_style))
